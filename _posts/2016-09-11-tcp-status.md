...
被动关闭的一方收到上面提到的对方的第一个FIN之后, 可能是马上就发送自己的close, (这个可能就和上一个ACK合并在一起发送了),也可以过一段时间再close(ACK还是会在收到FIN包之后马上回复)
...
### CLOSED
被关闭一方在收到最后个ACK之后, 关闭连接.对它来说, 这个连接就算是完全结束了, 像从来没有发生过一样.

### 多说几句TIME_WAIT

上面提到, 主动关闭一方会在最后停留在TIME_WAIT状态一段时间, 在这段时间内, 这个端口(IP+PORT)是不可用的.
