...
vmtouch可以方便的控制和诊断系统文件在系统Cache中的情况
...
- 来用查询Cache占用情况, 是用了mincore系统调用, 它会设置一个[]byte的数组, 长度是pagesize, 每一个字节代表这一页的情况.
