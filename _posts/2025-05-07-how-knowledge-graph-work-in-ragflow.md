...
1. 创建，使用 LLM（prompt如下），来提取 entity 和 relation，以及总结出 description。存到ES中。他们没有外键关联，纯粹的独立的字符串。
2. 搜索，使用 LLM 从用户问题中提取关键词和实体，然后在知识图谱中进行向量搜索，找到相关的边和节点。ragflow 中并没有继续按节点关系跳到其他文档。
